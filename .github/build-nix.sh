#!/usr/bin/env bash
# Build Nix itself against the staging prefix.
...
# explicitly or nixpkgs clang silently takes over the build.
...
# Build tools come from the nix profile — macos-build.yml installs the
...
for t in meson ninja cmake pkg-config bison flex jv; do
  command -v "$t" >/dev/null || { echo "missing build tool: $t (see macos-build.yml build-tools step)" >&2; exit 1; }
done
...
# Slim the payload: the .pkg is a runtime artifact. Dev headers,
...
# The meson-installed daemon plist too: nix-install writes its own
# (direct exec, no wait4path), so the template must not ship.
rm -rf "$PAYLOAD/opt/nix/Library"

# Populate the REAL /opt/nix so the baked absolute paths resolve during the
# test ladder (one build flavor: what ships is what's tested). The
