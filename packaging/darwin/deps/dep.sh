...
#
# Skip logic: staging/.built/<name> records "<version> <source> <build.sh sha256>".
# If it matches, the dep is skipped — a restored staging cache makes
# previously built deps free; editing a build.sh or changing a source
# invalidates its own stamp automatically.
set -euo pipefail
...
source=$(jq -er --arg n "$name" '.[$n].source' "$SOURCES/manifest.json")
want="$version $source $(shasum -a 256 "$deps_dir/$name/build.sh" | cut -d' ' -f1)"
