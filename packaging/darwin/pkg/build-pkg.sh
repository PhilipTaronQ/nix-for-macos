#!/usr/bin/env bash
# Assemble the macOS installer package (DESIGN.md §11).
#
...
# `nix-install install`, which applies the §11.2 plan for everything that
...
#
# Unsigned for now (DESIGN §4); notarization once a Developer ID exists.
set -euo pipefail
