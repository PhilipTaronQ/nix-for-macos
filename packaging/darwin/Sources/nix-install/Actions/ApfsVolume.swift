...
/// §11.2 step 3: the APFS volume that becomes /nix.
///
/// Wisdom inherited (§11.2/§11.4, upstream file pointers in DESIGN.md):
...
struct ApfsVolume: Codable {
    ...
    func revert(_ ctx: Context) throws {
        ...
        // Unmount first (tolerating "not mounted"), then delete with
        // retries — both operations race each other and the OS.
        if (info["MountPoint"] as? String)?.isEmpty == false {
            _ = try? ctx.runner.run(Self.diskutil, ["unmount", "force", volumeName])
        }
        let target = volumeUUID ?? volumeName
        try retrying(times: 10, delayMilliseconds: 500, ctx) {
            try ctx.runner.runChecked(Self.diskutil, ["apfs", "deleteVolume", target])
        }
    }
}
