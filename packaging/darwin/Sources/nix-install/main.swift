...
///   nix-install uninstall  remove Nix's OS integration; keep the store
///                          volume so a reinstall re-adopts it (soft)
///   nix-install plan       show the plan / ledger states, change nothing
///
/// Options:
///   --purge       with uninstall: also destroy the store volume and its
///                 data — a full teardown, not the default
...
let usage = """
usage: nix-install <install|uninstall|plan|repair> [--purge] [--root PATH] [--dry-run] [--payload-source PATH]
"""
...
do {
    ...
    case "uninstall":
        requireRootIfReal()
        try engine.uninstall(purge: purge)
        print(
            purge
                ? "uninstall complete (purged: store volume and data removed)"
                : "uninstall complete (store volume kept; reinstall re-adopts it, "
                    + "or 'uninstall --purge' to remove it)")
    case "plan":
        if store.exists {
