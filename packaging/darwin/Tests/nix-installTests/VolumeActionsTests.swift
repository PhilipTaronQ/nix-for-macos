...
final class VolumeActionsTests: XCTestCase {
    ...
    func testDefaultPlanInstallUninstallRoundTrip() throws {
        ...
        // The payload the .pkg (here: a source dir) provides. The daemon
        // plist is not part of it: DaemonService writes its own.
    ...
    /// The store layer survives a default (soft) uninstall and a reinstall
    /// re-adopts it; --purge removes it. Two file-only actions — one per
    /// layer — exercise the keep/revert split without the diskutil runner.
    func testSoftUninstallKeepsStoreLayerThenPurgeRemovesIt() throws {
        let plan: [Action] = [.syntheticConf(SyntheticConf()), .pathsD(PathsD())]
        let engine = Engine(store: LedgerStore(root: root), ctx: ctx)
        try engine.install(plan: plan)

        let store = ctx.path("/etc/synthetic.conf")  // store layer
        let integration = ctx.path("/etc/paths.d/nix")  // integration layer
        ...
        // Soft: integration reverted, store kept, ledger preserved.
        try engine.uninstall()
        XCTAssertFalse(
            FileManager.default.fileExists(atPath: integration.path), "integration reverted")
        XCTAssertTrue(
            FileManager.default.fileExists(atPath: store.path), "store layer kept")
        XCTAssertTrue(LedgerStore(root: root).exists, "ledger survives a soft uninstall")
        let mid = try LedgerStore(root: root).load()
        XCTAssertEqual(mid.actions[0].state, .completed, "store entry still completed")
        XCTAssertEqual(mid.actions[1].state, .uncompleted, "integration entry reverted")

        // Reinstall re-adopts: integration re-wired, store untouched.
        try engine.install(plan: plan)
        XCTAssertTrue(
            FileManager.default.fileExists(atPath: integration.path),
            "reinstall re-wires integration onto the surviving store")

        // Purge: store layer and ledger both gone.
        try engine.uninstall(purge: true)
        XCTAssertFalse(
            FileManager.default.fileExists(atPath: store.path), "purge removes the store layer")
        XCTAssertFalse(LedgerStore(root: root).exists, "purge removes the ledger")
    }
